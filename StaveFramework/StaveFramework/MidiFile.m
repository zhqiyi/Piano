        ...
        **/
        ...
        if (eventflag >= EventNoteOn && eventflag < EventNoteOn + 16) {
            ...
            if ([mevent controlNum] == 14) {
                ...
                if (preStart3 == 0 || [mevent startTime] - preStart3 > 40) {
                    if (preValue3 == -1) {
                        preValue3 = value;
            ...
            if ([mevent controlNum] == 15) {
                ...
                if (preStart4 == 0 || [mevent startTime] - preStart4 > 40) {
                    if (preValue4 == -1) {
                        preValue4 = value;
...
+(BOOL)writeToFile:(NSString*)filename withEvents:(Array*)eventlists
    ...
    for(int sectionnum = 0; sectionnum < dataLen/7;sectionnum++)
    {
        ...
        if (bFlag == 6) {
            int tmp = [midifile quarternote]%128;
            buf[4] = tmp;
            //            NSLog(@"buf[4] %i ", buf[4]);
            buf[5] = 0x4B;
        }else{
            int tmp1 = [midifile quarternote]/128;
            int tmp2 = [midifile quarternote]%128;
            buf[4] = tmp1 + 128;
            buf[5] = tmp2;
